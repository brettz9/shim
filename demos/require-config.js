...
require.config({
    ...
    config: {
        ...
        shim: { // IMPORTANT: This "shim" plugin property must be within the "config" property, as distinguished from the "shims" property at the top level for shimming non-AMD modules
            ...
            autoNamespace: 'main', // Currently the only possible value; namespaces one folder deep based on first part of file name preceding the first "." (though the file is still expected to contain the folder name: e.g., require('shim!Array.prototype.map'); would look for "Array/Array.prototype.map.js" relative to any base paths); I may add other types
